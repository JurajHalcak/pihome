...
function ZapisDoDB($teplota,$adresa,$verzia,$conn) {
  ...
  echo "pocet riadkov :".$riadkov." ";
  if (mysqli_num_rows($result) == 0) {
	  echo "Novy senzor. Adresa :".$adresa." a teplota: ".$teplota."<br />";
	  $query1 = "INSERT INTO nodes (node_id, child_id_1, name, last_seen, sketch_version) VALUES('{$adresa}', '0', 'Temperature Sensor', '{$date_time}', '{$verzia}')";
  ...
  elseif (mysqli_num_rows($result) == 1) {
	  $query1="SELECT id FROM nodes WHERE node_id = '{$adresa}'";
	  $result1 = $conn->query($query1);
	  $row = mysqli_fetch_array($result1);
	  $vysledok = $row['id'];
	  echo "Senzor id: ".$vysledok." uz existuje, updatujem hodnoty";
	  $query2 = "UPDATE nodes SET last_seen=now() WHERE node_id = '{$adresa}'";
	  $result2 = $conn->query($query2);
	  $query3 = "INSERT INTO messages_in(node_id, child_id, sub_type, payload, datetime) VALUES('{$adresa}', '0', '0', '{$teplota}', '{$date_time}')";
